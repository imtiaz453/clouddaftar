...
export async function adjustStockBalance(params: {
  ...
  await prisma.$transaction(async (tx) => {
    const before = await getOrCreateStockBalance(
      tx,
      params.productId,
      params.locationId,
      params.companyId,
    ...
    const movementType: StockMovementType =
      params.movementType ?? (params.direction === "IN" ? "ADJUSTMENT_IN" : "ADJUSTMENT_OUT");
...
export async function getProductStockSummary(
  ...
  const avgCost =
    balances.length > 0
      ? balances.reduce((s, b) => s + Number(b.averageCost) * Number(b.qtyOnHand), 0) /
        (totalOnHand || 1)
      : 0;
...
export async function receivePurchaseIntoStock(params: {
  locationId: string;
  productId: string;
  companyId: string;
  quantity: number;
...
export async function returnSaleStock(params: {
  locationId: string;
  productId: string;
  companyId: string;
  quantity: number;
...
export async function returnPurchaseStock(params: {
  locationId: string;
  productId: string;
  companyId: string;
  quantity: number;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const qtyOnHandAfter = before.qtyOnHand - params.quantity;
    if (qtyOnHandAfter < 0 && !params.allowNegative)
...
export async function reserveSaleStock(params: {
  locationId: string;
  productId: string;
  companyId: string;
  quantity: number;
...
export async function createOpeningBalance(params: {
  locationId: string;
  productId: string;
  companyId: string;
  quantity: number;
  ...
  return prisma.$transaction(async (tx) => {
    for (const item of data.items) {
      ...
      if (onHand < item.quantity) {
        ...
        throw new Error(
          `Insufficient stock for "${product?.name ?? item.productId}" at source location`,
        );
      }
    }
...
export async function issueStockTransfer(transferId: string, companyId: string, userId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of transfer.items) {
      ...
      const fromBefore = await getOrCreateStockBalance(
        tx,
        item.productId,
...
export async function receiveStockTransfer(transferId: string, companyId: string, userId: string) {
  ...
  if (!isOwner) {
    ...
    if (responsibleEmployeeId !== userId) {
      const responsibleName =
        transfer.destinationLocation.assignedEmployee?.name || "the assigned responsible employee";
      throw new Error(
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of transfer.items) {
      const qty = Number(item.quantity);
      const toBefore = await getOrCreateStockBalance(
        tx,
        item.productId,
      ...
      await tx.stockBalance.update({
        ...
        data: {
          qtyOnHand: toQtyOnHandAfter,
          qtyAvailable: calcAvailable(toQtyOnHandAfter, toBefore.qtyReserved),
          lastMovementAt: new Date(),
        },
      });
      await postStockLedger(tx, {
...
export async function instantTransfer(data: {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (fromQtyOnHandAfter < 0 && !data.allowNegative)
    ...
    await postStockLedger(tx, {
      ...
      notes: `Instant transfer to ${data.destinationLocationId}`,
      createdById: data.createdById,
    });
    await postStockLedger(tx, {
      productId: data.productId,
      ...
      reference: data.reference ?? null,
      referenceId: data.referenceId ?? null,
      notes: `Instant transfer from ${data.sourceLocationId}`,
      createdById: data.createdById,
    });
  });
}
...
export async function postStockAdjustment(adjustmentId: string, companyId: string, userId: string) {
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of adjustment.items) {
      ...
      const totalCost = Number(before.averageCost) * Number(before.qtyOnHand);
      const newTotalCost =
        direction === "IN" ? totalCost + qty * unitCost : totalCost - qty * unitCost;
      const newAvgCost = qtyOnHandAfter > 0 ? newTotalCost / qtyOnHandAfter : 0;
...
export async function getStockLedger(params: {
  ...
  if (params.locationIds) where.locationId = { in: params.locationIds };
...
export async function getInventoryDashboard(companyId: string) {
  const [productCount, locationCount, totalBalances, pendingTransfers, recentLedger, productLots] =
    await Promise.all([
      prisma.product.count({
        where: { companyId, deletedAt: null, isActive: true, isService: false },
      }),
      ...
      prisma.stockTransfer.count({
        where: { companyId, status: { in: ["DRAFT", "ISSUED", "PARTIALLY_RECEIVED"] } as any },
      }),
      prisma.stockLedger.findMany({
        where: { companyId },
        include: {
  ...
  const totalSkuWithStock = new Set(
    totalBalances.filter((b) => Number(b.qtyOnHand) > 0).map((b) => b.productId),
  ).size;
  ...
  const outOfStockCount = new Set(
    totalBalances
      .filter((b) => Number(b.qtyOnHand) === 0 && Number(b.product.minStock) > 0)
      .map((b) => b.productId),
  ).size;
  ...
  const damagedStockValue = damagedBalances.reduce(
    (s, b) => s + Number(b.qtyOnHand) * Number(b.averageCost),
    0,
  );

  // Employee custody stock value
  const employeeBalances = totalBalances.filter((b) => b.location.type === "EMPLOYEE_STORE");
  const employeeCustodyValue = employeeBalances.reduce(
    (s, b) => s + Number(b.qtyOnHand) * Number(b.averageCost),
    0,
  );
  ...
  return {
    ...
    stockByLocation: Array.from(stockByLocationMap.entries()).map(([locationId, data]) => ({
      locationId,
      ...data,
    })),
  };
...
export async function validateLocationRules(
  ...
  const errors: string[] = [];
  if (location.type === "MAIN_WAREHOUSE" && location.branchId)
    errors.push("Main warehouse should not be linked to a branch");
  if ((location.type === "BRANCH_STORE" || location.type === "POS_STORE") && !location.branchId)
    errors.push("Branch store must be linked to a branch");
  if (location.type === "EMPLOYEE_STORE" && !location.assignedEmployeeId)
    errors.push("Employee store must be linked to an employee");
