...
export function AccountingReportsClient() {
  ...
  const normalizeAgingData = (payload: any) => {
    const rows = payload?.rows ?? payload?.agingData ?? payload?.data ?? [];
    const summary = payload?.summary ?? rows.reduce(
      (acc: any, row: any) => ({
        ...
        totalOverdue:
          acc.totalOverdue +
          Math.max(0, Number(row.totalDue || 0) - Number(row.current || 0)),
      }),
      emptySummary,
    );
  ...
  const normalizeCollections = (payload: any) => ({
    rows: payload?.rows ?? payload?.payments ?? [],
  ...
  const fetchAgingReport = async () => {
    const res = await fetch(`/api/accounting/reports/aging?type=${reportType}`).then(r => r.json());
  ...
  const fetchCollectionReport = async () => {
    ...
    const res = await fetch(`/api/accounting/reports/collections?${params}`).then(r => r.json());
  ...
  const fetchOutstandingReport = async () => {
    const res = await fetch(`/api/accounting/reports/outstanding?type=${reportType === "RECEIVABLE" ? "CUSTOMER" : "SUPPLIER"}`).then(r => r.json());
  ...
  const COLORS = { current: "#10b981", days1to30: "#f59e0b", days31to60: "#f97316", days61to90: "#ef4444", days90plus: "#991b1b" };

  const exportColumnsAging = [
    { key: "entity", label: "Entity" }, { key: "current", label: "Current" },
    { key: "days1to30", label: "1-30 Days" }, { key: "days31to60", label: "31-60 Days" },
    { key: "days61to90", label: "61-90 Days" }, { key: "days90plus", label: "90+ Days" },
  ...
  const exportCollectionReport = () => {
    ...
    exportToCSV(collectionData.rows || [], [{ key: "date", label: "Date" }, { key: "amount", label: "Amount" }], `collections-${Date.now()}`);
    addToast({ title: "Exported", variant: "success" });
  };

  const exportOutstandingReport = () => {
    if (!outstandingData) return;
    exportToCSV(outstandingData.rows || [], [{ key: "name", label: "Name" }, { key: "totalDue", label: "Total Due" }], `outstanding-${Date.now()}`);
    addToast({ title: "Exported", variant: "success" });
  };

  const exportPaymentHistory = () => {
    if (!paymentHistory) return;
    exportToCSV(paymentHistory.rows || [], [{ key: "date", label: "Date" }, { key: "entity", label: "Entity" }, { key: "type", label: "Type" }, { key: "amount", label: "Amount" }, { key: "method", label: "Method" }, { key: "reference", label: "Reference" }], `payment-history-${Date.now()}`);
    addToast({ title: "Exported", variant: "success" });
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="aging" className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <select value={reportType} onChange={e => setReportType(e.target.value)} className="h-9 w-full rounded-lg border border-input bg-background px-3 text-sm sm:w-auto">
              <option value="RECEIVABLE">Receivables (Customer)</option>
              <option value="PAYABLE">Payables (Supplier)</option>
            </select>
            <Button variant="outline" size="sm" className="w-full sm:w-auto" onClick={exportAgingReport}>
          ...
          {agingData && (
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
                {[{ label: "Current", value: agingData.summary.totalCurrent, color: "text-emerald-600" },
                  { label: "1-30 Days", value: agingData.summary.total1to30, color: "text-amber-600" },
                  { label: "31-60 Days", value: agingData.summary.total31to60, color: "text-orange-600" },
                  { label: "61-90 Days", value: agingData.summary.total61to90, color: "text-red-600" },
                  { label: "90+ Days", value: agingData.summary.total90plus, color: "text-red-800" },
                  { label: "Total", value: agingData.summary.totalOverdue, color: "text-destructive" },
                ].map(s => (
                  <Card key={s.label}><CardHeader className="pb-2"><CardTitle className="text-xs text-muted-foreground">{s.label}</CardTitle></CardHeader><CardContent><p className={`text-lg font-bold ${s.color}`}>{formatCurrency(s.value)}</p></CardContent></Card>
                ))}
              </div>
              <Card className="h-72">
                <CardHeader><CardTitle>Aging Distribution</CardTitle></CardHeader>
                <CardContent className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={[
                      { name: "Current", value: agingData.summary.totalCurrent, fill: COLORS.current },
                      { name: "1-30", value: agingData.summary.total1to30, fill: COLORS.days1to30 },
                      { name: "31-60", value: agingData.summary.total31to60, fill: COLORS.days31to60 },
                      { name: "61-90", value: agingData.summary.total61to90, fill: COLORS.days61to90 },
                      { name: "90+", value: agingData.summary.total90plus, fill: COLORS.days90plus },
                    ]}>
        ...
        <TabsContent value="collection" className="space-y-4">
          <div className="flex items-end gap-3">
            <div><Label className="text-xs">From</Label><Input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="h-8" /></div>
            <div><Label className="text-xs">To</Label><Input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="h-8" /></div>
        ...
        <TabsContent value="outstanding" className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <select value={reportType} onChange={e => setReportType(e.target.value)} className="h-9 rounded-lg border border-input bg-background px-3 text-sm">
              ...
              <Table>
                ...
                <TableBody>
                  {outstandingData.rows?.length === 0 ? (
                    <TableRow><TableCell colSpan={2} className="py-8 text-center text-sm text-muted-foreground">No outstanding {reportType === "RECEIVABLE" ? "receivables" : "payables"}</TableCell></TableRow>
                  ) : outstandingData.rows?.map((r: any, i: number) => (
                    <TableRow key={i}>
                      <TableCell className="font-medium">{r.name}</TableCell>
                      <TableCell className="text-right font-bold">{formatCurrency(r.totalDue)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
              ...
              <Table>
                ...
                <TableBody>
                  {paymentHistory.rows?.length === 0 ? (
                    <TableRow><TableCell colSpan={6} className="py-8 text-center text-sm text-muted-foreground">No payment history found</TableCell></TableRow>
                    ...
                    <TableRow key={i}>
                      ...
                      <TableCell><Badge variant={r.type === "RECEIVED" ? "success" : "destructive"}>{r.type}</Badge></TableCell>
                      <TableCell className={`text-right font-medium ${r.type === "RECEIVED" ? "text-green-600" : "text-red-600"}`}>{formatCurrency(r.amount)}</TableCell>
