...
export function SupplierLedgerClient({ supplierId, initialData }: SupplierLedgerClientProps) {
  ...
  const loadPage = useCallback(
    async (newPage: number) => {
      ...
      try {
        ...
        const res = await fetch(`/api/accounting/ledger/supplier/${supplierId}?${params}`);
        if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold">{data.supplier.name}</h2>
          <p className="text-sm text-muted-foreground">
            {data.supplier.email || data.supplier.phone || ""}
          </p>
        </div>
