...
export async function getLocationsForSelect(params?: {
  ...
  scope?: "all" | "transfer-source" | "transfer-destination";
}) {
  ...
  // Transfer destination must show all active stores/locations so a POS cashier
  // can send stock to any store. Source is restricted below.
  if (params?.scope === "transfer-source" && user.role !== "OWNER") {
    const membership = await prisma.companyMembership.findFirst({
      where: { companyId, userId: user.id, isActive: true },
      select: { branchId: true },
    });

    const sourceScope: Record<string, unknown>[] = [
      { assignedEmployeeId: user.id },
    ];

    if (membership?.branchId) {
      sourceScope.push({ branchId: membership.branchId });
    }

    where.OR = sourceScope;
  }
