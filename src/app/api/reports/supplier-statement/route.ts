...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!supplierId) return errorResponse("supplierId is required");

    const supplier = await prisma.supplier.findFirst({
      where: { id: supplierId, companyId: user.companyId, deletedAt: null },
    ...
    if (format === "pdf") {
      const html = generateStatementHtml({
        ...
        currencySymbol: supplier.company.currencySymbol || "Rs",
        transactions,
      });
      ...
      return new Response(new Uint8Array(pdfBuffer), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="${pdfFilename("supplier-statement", supplier.name)}"`,
